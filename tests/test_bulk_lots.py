...
class BulkLotsServiceTest(unittest.TestCase):
	def test_off_records_only_successful_lot_ids(self):
		settings = {"bulk_lots": {"disabled_lot_ids": []}}
		service = BulkLotsService(fake_cardinal({"1": True, "2": True}, {"2"}), settings, lambda: None)
	...
	def test_on_skips_manually_enabled_lot_without_saving(self):
